...
import { ShowAll } from "../eachCategory/ShowAll";
...
const Query = () => {
  ...
  useEffect(() => {
    const fetchQuery = async () => {
        try {
            let add = `https://gnews.io/api/v4/search?q=${keyword}&apikey=${import.meta.env.VITE_REACT_GNEWS_API_KEY}`
            const res = await axios.get(add)
            setArticles(res.data.articles)
        } catch(err) {
            console.log("Cant fetch Query")
        }
  ...
  return (
    <section className="each-category">
      ...
      {articles?.slice(indexOfFirstRecord,indexOfLastRecord).map((news,i) => {
        return <ShowAll key={i} id={i+indexOfFirstRecord} time={news.publishedAt} image={news.image} title={news.title} name={news.source.name} category='query' />
      })}
      <div className="page">
        <nav>
